...
function remove(gigId) {
  // return storageService.remove('gig', gigId)
  return httpService.delete(`gig/${gigId}`);
}

async function update(gig) {
  // await storageService.put('gig', gig)
  gig = await httpService.put(`gig/${gig._id}`, gig);
  // Handle case in which admin updates other gig's details
  if (getLoggedinGig()._id === gig._id) _saveLocalGig(gig);
  return gig;
}

function _saveLocalGig(gig) {
